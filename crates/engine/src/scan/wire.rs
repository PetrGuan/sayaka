...
//! Shared v1 scan/progress JSON for CLI and native hosts.
...
mod tests {
    use super::*;
    use crate::scan::{ScanCode, ScanStatus, ScanTaskId};
    ...
    #[cfg(windows)]
    #[test]
    fn synthetic_unpaired_utf16_path_serialization_is_lossless() {
        use std::ffi::OsString;
        use std::os::windows::ffi::OsStringExt;
        let path = PathBuf::from(OsString::from_wide(&[67, 58, 92, 0xd800]));
        let mut bytes = Vec::new();
        line(&mut bytes, &NativePath(&path)).unwrap();
