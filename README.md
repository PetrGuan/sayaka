...
| Native-client bindings | Narrow C ABI for background scan/progress/cancel/result/release; local C/Swift consumers | Read-only first slice; Windows cross-check is not native host acceptance, and effects/status bindings remain absent |
...
crate now provides a [read-only scan integration API](docs/BINDINGS.md), not a
complete native SDK.
...
| `sayaka-bindings` | Versioned read-only C scan ABI and C/Swift host examples |
