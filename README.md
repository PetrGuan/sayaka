...
| Rules and `clean` | CPython source-backed `.pyc` and OpenJDK `javac` source-backed `.class`; clean-only persistent exclusions; read-only `purge` preview of marker-bound rebuildable project artifacts | Two narrow rules, not a broad cache/log/project-artifact cleanup catalog; no directory effects |
...
| Applications | `apps` inventory, `apps-related` association preview, opt-in running-process attribution and single-bundle `uninstall` Trash with typed confirmation | Related data untouched; no multi-copy flows, programmatic restore or related-data removal |
