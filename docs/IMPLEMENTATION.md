...
| T6: bindings and release contract | `crates/bindings/`, C/Swift host examples, integration documentation | Read-only scan/directory queries and macOS installer discovery/selection-check ABI; native App/Windows runtime, broader APIs and binary delivery remain | Shared scan/lifecycle/index/installer checks; independent native platform evidence |
...
| T8: full cleanup workflows | Rules, installer discovery/selection and CLI flows | Two-rule clean and narrow installer workflow implemented; directory assessment is ModelOnly, not purge execution; read-only `purge` preview groups marker-bound rebuildable artifacts | T5 and T7; new contracts for directory actions |
